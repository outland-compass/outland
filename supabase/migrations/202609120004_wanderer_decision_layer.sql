...
create policy "mobile_candidate_specs_select_authenticated"
  on land.mobile_candidate_specs for select
  to authenticated
  using (true);

create policy "mobile_candidate_specs_write_admin"
  on land.mobile_candidate_specs for all
  to authenticated
  using (core.is_admin())
...
select c.id, 2018, 52500, 5990, 3500,
       'Gas heating / Winterpaket (listing; exact heater model unverified)',
       null,
       '2 x 95 Ah house batteries listed; chemistry unverified',
       true,
       'Cassette WC / exact model unverified',
       'FWD + Traction+',
       'Winterpaket listed; full frost protection not yet verified'
from land.candidates c
join shared.worlds w on w.id = c.world_id
where w.code = 'WANDERER'
  and c.title ilike 'Pössl Summit 600 Plus%'
on conflict (candidate_id) do update set
  ...
  battery_summary = excluded.battery_summary,
  indoor_shower = excluded.indoor_shower,
  toilet_type = excluded.toilet_type,
  drivetrain = excluded.drivetrain,
  winter_summary = excluded.winter_summary,
  updated_at = now();

insert into land.candidate_economics (
  candidate_id, likely_purchase_price, transaction_legal_costs,
  immediate_infrastructure_safety_capex, minimum_usable_state_capex,
  first_unit_fitout_capex, phase1_capital_excluding_purchase,
  total_phase1_capital, assumptions
)
...
where w.code = 'WANDERER'
  and c.title ilike 'Pössl Summit 600 Plus%'
on conflict (candidate_id) do update set
  likely_purchase_price = excluded.likely_purchase_price,
  transaction_legal_costs = excluded.transaction_legal_costs,
  immediate_infrastructure_safety_capex = excluded.immediate_infrastructure_safety_capex,
  minimum_usable_state_capex = excluded.minimum_usable_state_capex,
  first_unit_fitout_capex = excluded.first_unit_fitout_capex,
  phase1_capital_excluding_purchase = excluded.phase1_capital_excluding_purchase,
  total_phase1_capital = excluded.total_phase1_capital,
