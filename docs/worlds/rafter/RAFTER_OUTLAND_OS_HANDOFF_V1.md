...
- local device interaction for the automatic main entrance, light/audio/cabinet where explicitly needed.
...
Physical boundary: **Front Deck → automatic main entrance → River Room**.

Required behavior:
- read Passport credential;
- determine whether temporary access is valid;
- on valid access, command the local entrance controller to unlock/open the automatic main entrance;
- deny normal automated entry for invalid/expired/revoked credentials;
...
The automatic entrance is the normal guest-entry mechanism, but it is not the sole safe-exit mechanism. Exact door actuator, lock, controller, power and emergency-release implementation remain physical `TO VERIFY` items owned by RAFTER.
...
- automatic main-entrance lock/door actuator and local entrance controller;
...
- Loss of internet must not prevent safe exit; safe egress must not depend on Passport/NFC, network/cloud or normal powered door automation.
- Mystery events may queue for later synchronization.
- Mystery may degrade gracefully if device/cloud state is unavailable.
- Mechanical/manual emergency egress and operator override must exist for the automatic main entrance; mechanical operator override must also exist for cabinet service.
...
3. Entrance terminal accepts a valid credential and commands the local controller to open the automatic Front Deck → River Room main entrance; invalid/expired/revoked credentials are denied normal automated entry.
...
## 13. FAIL conditions
- Requires cloud connectivity for valid-stay normal entry during the intended offline case, or makes safe exit dependent on Passport/NFC, network/cloud or powered door automation.
...
| Physical platform / layout / BLOCKS | Full context | Only constraints relevant to access/SENSE/Mystery, including Front Deck → automatic entrance → River Room boundary | Store under `docs/worlds/rafter/` |
