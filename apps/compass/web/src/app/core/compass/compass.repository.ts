...
export interface MobileCandidateSpec { candidate_id:string; model_year:number|null; mileage_km:number|null; length_mm:number|null; gross_weight_kg:number|null; heating_type:string|null; solar_w:number|null; battery_summary:string|null; indoor_shower:boolean|null; toilet_type:string|null; drivetrain:string|null; winter_summary:string|null; }

export interface ImportedListing { source_name:string|null; source_url:string; source_listing_id:string|null; title:string|null; location:string|null; price:number|null; currency:string|null; area_m2:number|null; description:string|null; image_url:string|null; }
interface PostgrestError { code?:string; message:string; }
type PostgrestResponse<T> = { data:T|null; error:PostgrestError|null };
const FUTURE_JWT_RETRY_DELAY_MS = 750;
function isFutureJwtError(error:PostgrestError|null):boolean { return error?.code === 'PGRST303' || error?.message.includes('JWT issued at future') === true; }
export function extractSourceImage(snapshot:unknown):string|null { if(!snapshot||typeof snapshot!=='object')return null; const payload=(snapshot as Record<string,unknown>)['raw_payload']; if(!payload||typeof payload!=='object')return null; const value=(payload as Record<string,unknown>)['image_url']; return typeof value==='string'&&value.trim()?value:null; }

@Injectable({ providedIn:'root' })
export class CompassRepository {
  constructor(private readonly supabase:SupabaseService) {}
  private get db(){return this.supabase.client;} private get land(){return this.supabase.client.schema('land');} private get shared(){return this.supabase.client.schema('shared');}
  ...
  async mobileSpecs(candidateIds:string[]):Promise<MobileCandidateSpec[]>{if(!candidateIds.length)return[];return this.data(this.land.from('mobile_candidate_specs').select('candidate_id,model_year,mileage_km,length_mm,gross_weight_kg,heating_type,solar_w,battery_summary,indoor_shower,toilet_type,drivetrain,winter_summary').in('candidate_id',candidateIds)) as unknown as Promise<MobileCandidateSpec[]>;}
  ...
  private async functionErrorMessage(error:unknown):Promise<string>{const context=(error as{context?:unknown})?.context;if(context instanceof Response){try{const body=await context.clone().json();if(body&&typeof body==='object'&&'error'in body)return String((body as{error:unknown}).error);}catch{/* fall through */}}return error instanceof Error?error.message:'Listing could not be imported.';}
}