...
describe('RadarPage thumbnail mapping', () => {
    ...
    const result = mapRadarSources([
      {
        ...
        source_url: 'https://listing/a',
        image_url: 'https://image/a.jpg',
      },
    ]);
  ...
  it('keeps the first valid image for each candidate', () => {
    const result = mapRadarSources([
      { candidate_id: 'candidate-a', source_url: null, image_url: '   ' },
      {
        candidate_id: 'candidate-a',
        source_url: 'https://listing/a',
        image_url: 'https://image/a-first.jpg',
      },
      {
        candidate_id: 'candidate-a',
        source_url: 'https://listing/a-2',
        image_url: 'https://image/a-second.jpg',
      },
  ...
  it('marks only the candidate whose image failed', () => {
    ...
    (RadarPage.prototype.onCandidateImgError as Function).call(
      { candidateImgFailed },
      'candidate-a',
    );
...
describe('RadarPage thumbnail rendering', () => {
  const row = (id: string, worldCode = 'GREENHILL'): RadarRow =>
    ({
      id,
      title: `Candidate ${id}`,
  ...
  async function createPage(
    rows: RadarRow[],
    sources: { candidate_id: string; source_url: string | null; image_url: string | null }[],
    signals: Signal[] = [],
  ) {
    const worldCodes = [
      ...new Set(
        rows.map((candidate) => candidate.world_code).filter((code): code is string => !!code),
      ),
    ];
    const worlds = worldCodes.map(
      (code) => ({ id: code.toLowerCase(), code, name: code, radar_enabled: true }) as World,
    );
    const repository = {
      radar: vi.fn().mockResolvedValue(rows),
      worlds: vi.fn().mockResolvedValue(worlds),
  ...
  it('renders an image when image_url is available', async () => {
    ...
    expect(fixture.nativeElement.querySelector('.thumb img')?.getAttribute('src')).toBe(
      'https://image/a.jpg',
    );
  });
  ...
  it('shows a candidate-specific placeholder after an image error', async () => {
    const { fixture } = await createPage(
      [row('candidate-a'), row('candidate-b')],
      [
        { candidate_id: 'candidate-a', source_url: null, image_url: 'https://image/a.jpg' },
  ...
  it('recomputes candidate results when the World filter changes through the UI', async () => {
    const { fixture, page } = await createPage(
      [row('greenhill', 'GREENHILL'), row('work-a', 'OUTLAND_WORK'), row('work-b', 'OUTLAND_WORK')],
      [],
    );
    expect(page.filtered().length).toBe(3);
    const worldSelect = fixture.nativeElement.querySelectorAll(
      '.toolbar select',
  ...
  it('renders a Signal image from raw_payload.image_url', async () => {
    ...
    fixture.detectChanges();
    expect(fixture.nativeElement.querySelector('.signal-thumb img')?.getAttribute('src')).toBe(
      'https://image/a.jpg',
    );
  });
  ...
  it('does not suppress another Signal after one image fails', async () => {
    const { fixture, page } = await createPage(
      [],
      [],
      [signal('signal-a', 'https://image/a.jpg'), signal('signal-b', 'https://image/b.jpg')],
    );
    page.panel.set('signals');
    fixture.detectChanges();
    fixture.nativeElement
      .querySelectorAll('.signal-thumb img')[0]
