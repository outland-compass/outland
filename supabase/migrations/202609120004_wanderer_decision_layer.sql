...
on conflict (candidate_id) do update set model_year=excluded.model_year,mileage_km=excluded.mileage_km,length_mm=excluded.length_mm,gross_weight_kg=excluded.gross_weight_kg,heating_type=excluded.heating_type,solar_w=excluded.solar_w,battery_summary=excluded.battery_summary,indoor_shower=excluded.indoor_shower,toilet_type=excluded.toilet_type,drivetrain=excluded.drivetrain,winter_summary=excluded.winter_summary,updated_at=now();

-- Existing generated columns calculate phase1_capital_excluding_purchase and total_phase1_capital.
insert into land.candidate_economics (candidate_id,likely_purchase_price,transaction_legal_costs,immediate_infrastructure_safety_capex,minimum_usable_state_capex,first_unit_fitout_capex,assumptions)
select c.id,38500,1500,1000,0,2000,'WANDERER Ready-to-Wander working estimate: target purchase €38.5k + ~€1.5k import/registration + ~€1k initial service/safety reserve + ~€2k minimal OUTLAND/off-grid retrofit. Verify before negotiation.'
from land.candidates c join shared.worlds w on w.id=c.world_id
where w.code='WANDERER' and c.title ilike 'Pössl Summit 600 Plus%'
on conflict (candidate_id) do update set likely_purchase_price=excluded.likely_purchase_price,transaction_legal_costs=excluded.transaction_legal_costs,immediate_infrastructure_safety_capex=excluded.immediate_infrastructure_safety_capex,minimum_usable_state_capex=excluded.minimum_usable_state_capex,first_unit_fitout_capex=excluded.first_unit_fitout_capex,assumptions=excluded.assumptions,updated_at=now();