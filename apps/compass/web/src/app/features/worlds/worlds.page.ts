    ...
    <div class="world-grid">
      @for(world of worlds();track world.id){
        <article [class.disabled]="!world.radar_enabled">
          ...
          @if(editingId()===world.id){
            <div class="hunt-editor">
              <h3>LAND HUNT</h3>
              <label>Development track<input [(ngModel)]="form().developmentTrack" placeholder="PERMANENT / WILDLAND_POD" /></label>
              <div class="field-grid">
                ...
                <label>Hard / exceptional max €<input type="number" [(ngModel)]="form().priceHardMax" /></label>
...
export default class WorldsPage {
  ...
  edit(world:World){const profile=this.profile(world);const hunt=this.hunt(world);const price=this.object(hunt['target_land_price_eur']);const area=this.object(hunt['preferred_area_m2']);this.error.set('');this.success.set('');this.form.set({developmentTrack:this.text(profile['development_track']),priceMin:this.num(price['min']),pricePreferredMax:this.num(price['preferred_max']),priceHardMax:this.num(price['hard_max'])??this.num(price['exceptional_max']),areaMin:this.num(area['min']),areaMax:this.num(area['max']),walkToSea:this.num(hunt['ideal_walk_to_sea_m']),legalPath:this.text(hunt['legal_path']),phaseSequence:this.text(hunt['phase_sequence'])});this.editingId.set(world.id)}
  ...
  async save(world:World){
    ...
    try{
      ...
      const price:JsonObject={...oldPrice};this.assignNumber(price,'min',f.priceMin);this.assignNumber(price,'preferred_max',f.pricePreferredMax);delete price['hard_max'];delete price['exceptional_max'];if(f.priceHardMax!=null){price['hard_max']=f.priceHardMax}
  ...
  priceRange(world:World){const p=this.object(this.hunt(world)['target_land_price_eur']);const min=this.num(p['min']);const preferred=this.num(p['preferred_max']);const hard=this.num(p['hard_max'])??this.num(p['exceptional_max']);if(min==null&&preferred==null&&hard==null)return '—';return `${this.money(min)} – ${this.money(preferred??hard)}${hard!=null&&hard!==preferred?` (hard ${this.money(hard)})`:''}`}
  ...
  private emptyForm():HuntForm{return{developmentTrack:'',priceMin:null,pricePreferredMax:null,priceHardMax:null,areaMin:null,areaMax:null,walkToSea:null,legalPath:'',phaseSequence:''}}
